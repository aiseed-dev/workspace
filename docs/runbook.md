...
コマンドだけ素早く思い出したいときは [cheatsheet.md](cheatsheet.md)（値が埋まった一画面）。
...
| 台帳ランチャー（コマンド・URL を即引く） | 手元 | `python3 scripts/daicho.py`（正は cheatsheet.md を直接読む。台帳ファイルは持たない） | [scripts/daicho.py](../scripts/daicho.py) |
